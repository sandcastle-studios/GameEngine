...
namespace FilePackager.ViewModels
{
	public class ManualPackagingInfoViewModel : ViewModelBase
	{
        ...
        {
            get { return _manualPackagingInfo.BlockSizeRSX; }
            set { SetValue(_manualPackagingInfo, "BlockSizeRSX", value); }
        }
        ...
        public bool CanBeRSX
        {
            get
            {
                return ProjectViewModel.Current.BasePlatform == AK.Wwise.InfoFile.SoundBanksInfoBasePlatform.PS3;
            }
        }
        ...
        internal void CreateRSXPackages()
        {
            using (new UndoManager.EventGroupScope())
            {
                ...
                // Create a package for every Soundbank containing RSX streamed files
                foreach (ContentItemViewModel item in items.Where(i => i.FileType == FileType.SoundBank))
                {
                    if (item.ReferencedStreamedFiles.Any(r => r.FileType == FileType.StreamedFileRSX && (r.Language == "SFX" || r.Language == item.Language)))
                    {
                        string packageName = Path.GetFileNameWithoutExtension(item.FileName) + ".RSX.pck";

                        // Append language
                        if (item.Language != "SFX")
                            packageName = Path.Combine(item.Language, packageName);

                        // Append the RSX directory
                        if (CommandLineViewModel.Instance.IsGenerateMode &&
                            !string.IsNullOrEmpty(CommandLineViewModel.Instance.RSXPackagePath))
                        {
                            packageName = Path.Combine(CommandLineViewModel.Instance.RSXPackagePath, packageName);
                            packageName = FilePackager.Base.Extensions.PathCanonicalize(packageName);
                        }

                        // This soundbank contains RSX files, look if a package already exist with this name
                        if (!Packages.Any(p => p.Name == packageName))
                        {
                            Package package = new Package() { Name = packageName };

                            package.IsRSX = true;
                            package.Items.Add(new PackageContentItem(item.Id, item.Language, item.FileName));
                            _manualPackagingInfo.Packages.Add(package);
                        }
                    }
                }
                    ...
                    foreach (ContentItemViewModelReference item in items)
                    {
                        ...
                        switch (item.ContentItem.FileType)
                        {
                            ...
                            case FileType.StreamedFileRSX:
                                packageItem.InclusionMode = InclusionMode.StreamsRSX;
                                break;
