	...
	public class ContentItemViewModel : ViewModelBase
	{
        ...
        public long Size
        {
            get
            {
                if (_size == -1)
                {
                    ...
                    else if (FileType == FileType.StreamedFile || FileType == FileType.StreamedFileRSX ||  FileType == FileType.LooseMedia )
                    {
		...
		public FileType FileType 
		{
			get
			{
                ...
                else if (_fileDescriptor.GetType() == typeof(AK.Wwise.InfoFile.SoundBanksInfoSoundBanksSoundBank))
					return FileType.SoundBank;

                else if (_fileDescriptor.GetType() == typeof(AK.Wwise.InfoFile.SoundBanksInfoStreamedFilesFile))
                     return ( (_fileDescriptor as AK.Wwise.InfoFile.SoundBanksInfoStreamedFilesFile).RSX ) ? FileType.StreamedFileRSX : FileType.StreamedFile;
                
                else if (_fileDescriptor.GetType() == typeof(AK.Wwise.InfoFile.SoundBanksInfoMediaFilesNotInAnyBankFile))
