...
namespace FilePackager.ViewModels
{
    public class PackageViewModel : ViewModelBase, IDataErrorInfo, IRenamable
	{
        ...
        {
            if (e.PropertyName == "CanBeRSX")
            {
                OnPropertyChanged("IsRSX");
            }
        }
        ...
        public bool IsRSX
        {
            get 
            {
                if (!ProjectViewModel.Current.ManualPackagingInfo.CanBeRSX)
                    return false;

                return _package.IsRSX; 
            }
            set { SetValue(_package, "IsRSX", value); }
        }
