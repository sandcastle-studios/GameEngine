...
bool InputMgr::AddDevice( GUID in_guidInstance )
{
	...
	m_pUInput->AddDevice( (AkUInt16) newInputObject.iPlayerIndex, UGDeviceType_GAMEPAD, newInputObject.pDevice );
...
)
{
	...
	m_InputObjects.push_back( newInputObject );
	sort( m_InputObjects.begin(), m_InputObjects.end(), CompareInputObjects );
	m_pUInput->AddDevice( 0, UGDeviceType_KEYBOARD, newInputObject.pDevice );
